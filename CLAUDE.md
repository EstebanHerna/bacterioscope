...
**Estado real, no la visión de arriba:** hoy el sistema detecta cada disco (Hough, confiable) y mide la zona (Otsu con exclusión del disco, ver F3), pero la lectura automática del antibiótico impreso no es usable todavía (F2 -- el modelo de 29 clases no alcanza un umbral defendible) y la asignación de antibiótico se hace manualmente o por posición de panel fijo (`panels/manager.py`, que sí funciona y no depende de datos de entrenamiento). No describas el sistema como "sin intervención humana" en tiempo presente sin esta salvedad -- ver la fila F2 en la tabla de fases y `docs/LIMITACIONES.md` sección 2.
...
**Full automation of antibiotic label reading is the long-term target**, not yet reached (see F2). The Phase 2 YOLOv8 model is meant to detect each disk and read the printed antibiotic abbreviation from its surface, but the 29-class model trained so far is not usable at any defensible confidence threshold. Until it is, `panels/manager.py` (fixed panel YAML + angular position assignment) is a real, working, data-independent way to resolve antibiotic identity -- not merely a stand-in for the manual Streamlit selectbox. Don't describe either of these as "temporary" in a way that implies they don't work; they do, today, and are what a lab would actually use.
...
**Calibration reference: plate rim, not the disk.** This reverses an earlier design intent recorded here. Phase 3 was meant to calibrate px/mm using the physical disk diameter (6 mm standard) instead of the plate rim, on the theory that the disk edge is always sharp and well-contrasted. Measured directly (see F3): disk-based calibration, even independently and correctly measured (`calibration.py::refine_disk_radius_px()`), underperforms plate-rim calibration (28.8% vs 32.9% identity-matched EA) -- not a bug, a measurement-theory limit, since the same few pixels of edge noise are a much larger relative error against a ~50-60px disk than a ~550-600px plate. `use_disk_calibration` stays `False`. Do not re-assert the disk is "more robust" without re-reading this finding; see `docs/VALIDATION_REPORT.md` and `docs/LIMITACIONES.md`.

## Phase status

| Phase | Scope | Status |
|---|---|---|
| F0 | End-to-end pipeline: Hough disk detection, Otsu+watershed zone segmentation, CLSI M100-Ed33 2023 classifier (15 antibiotics, Enterobacteriaceae), Streamlit demo, CLI, FastAPI, evaluation module (CA/EA/VME/ME/mE ISO 20776-2), design system (Clinical Slate tokens, dark/light CSS), traceability fields (analysis_id UUID, image SHA-256, commit hash, CLSI edition, stage timings), 9-section scientific HTML plate reports, Bland-Altman measurement validation, panel configuration, batch processing, 261 tests (94.9%+ coverage), CI green on Python 3.10/3.11/3.12 | **Complete** |
...
bacterioscope/
    ...
    tests/                        <- mirrors src/ structure, 261 tests
...
- Dryad/UZH (Egli et al., 2023): 225 Gram-negative isolates. Download: `python scripts/download_data.py`.
