...
def _write_report(
    ...
    lines += [
        ...
        "docs/ROADMAP.md / CLAUDE.md). An earlier round of this report "
        "claimed it was not a proven replacement for Hough, based on a "
        "comparison at an untuned conf=0.5 (60.0% exact disk-count match "
        "vs Hough's 73.8% on the 80 real UZH photos this report "
        "evaluates). That was wrong: a confidence sweep found a sharp "
        "optimum around conf=0.27-0.30 where the single-class model "
        "matches or slightly beats Hough (76-77.5% exact match), with "
        "performance collapsing outside a fairly narrow band. Hough stays "
        "the pipeline's default regardless -- not because it measures "
        "better, but because switching the default detector is a "
        "deployment decision (added torch/ultralytics dependency weight, "
        "model-load latency, and a real threshold-sensitivity risk Hough "
        "does not have) that has not been made yet. Disk *identity* still "
        "resolves through panel position, not label reading, either way.",
